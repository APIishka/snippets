...
export default function MasonryTextSnippets({ items, onEdit, isAuthenticated }) {
  ...
  const estimateHeight = useCallback((item) => {
    const totalChars = (item.text || '').length + (item.notes || '').length;
    const baseHeight = 56;
    const charsPerLine = 48;
    const lineHeight = 20;
    const numLines = Math.min(16, Math.max(2, Math.ceil(totalChars / charsPerLine)));
    return baseHeight + numLines * lineHeight;
  }, []);

  const distributedItems = useMemo(() => {
    if (columnCount === 1) {
      return items.map((item) => ({ item, column: 0 }));
    }
  ...
  return (
    <>
      <style>{`
        .masonry-textsnippets-container { position: relative; width: 100%; }
        .masonry-textsnippets-item { transition: top 0.2s ease, left 0.2s ease; }
      `}</style>
      <div
        ref={containerRef}
        className="masonry-textsnippets-container"
        style={
